...
const xpath = (data) => {
  ...
  var sames = [].filter.call(data.parentNode.children, function (x) { return x.tagName == data.tagName });

  return xpath(data.parentNode) + '/' + data.tagName.toLowerCase() + (sames.length > 1 ? '['+([].indexOf.call(sames, data) + 1) + ']' : '');
}